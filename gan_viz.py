...
from mnist_gan import MnistGenerator, MnistDiscriminator

image = None
orig_img = None
should_update = True

generator = torch.load("./saved_models/run_1/generator.pt", map_location=torch.device('cpu'))
discriminator = torch.load("./saved_models/run_1/discriminator.pt", map_location=torch.device('cpu'))
...
sliders = [tk.Scale(subframe, from_=-5.0, to_=5.0, resolution=0.05, length=290, orient=tk.HORIZONTAL, command=update_canvas if i != z_shape-1 else update_and_enable_updates) for i in range(z_shape)]
